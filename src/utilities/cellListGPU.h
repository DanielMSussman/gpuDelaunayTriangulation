...
class cellListGPU
    ...
    public:
        ...
        cellListGPU(std::vector<double> &points);
        //! constructor with points, a box, and a size for the underlying grid
        cellListGPU(double a, std::vector<double> &points, periodicBoundaries &bx);

        //!Set the object's points to a given vector
        void setParticles(const std::vector<double> &points);
        //!Set the object's points to a given vector of double2s
        void setParticles(const std::vector<double2> &points);
        ...
        void getCellNeighbors(int cellIndex, int width, std::vector<int> &cellNeighbors);
        //! given a target cell and a width, get all cell indices that sit on the requested shell
        void getCellShellNeighbors(int cellIndex, int width, std::vector<int> &cellNeighbors);
